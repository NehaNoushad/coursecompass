...
function downloadReport(answers: QuizAnswers, result: Recommendation) {
  ...
  win.document.close();
  // Give the new window 400 ms to render before opening the print dialog.
  // Calling win.print() immediately after document.close() can race the
  // browser's layout pass and show a blank preview on some browsers.
          ...
          <SectionHead
            ...
            sub={
              result.courses.length > MAX_GRID_COURSES
                ? `Showing ${MAX_GRID_COURSES} of ${result.courses.length}, best first.`
                : `${result.courses.length} matches, best first.`
            }
          />
          <View style={rs.grid}>
